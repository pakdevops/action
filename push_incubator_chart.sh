...
helmv3 repo add helmrepo ${HELM_REPO} --username=${HELM_USER} --password=${HELM_PASSWORD}
helmv3 plugin install https://github.com/chartmuseum/helm-push

if [[ ${IMAGE_TAG} =~ "mixedmanual" ]]; then
  helmv3 cm-push ${APP_CHART_NAME}-${BRANCH_NAME} helmrepo --force || exit 1
else
  helmv3 cm-push ${APP_CHART_NAME} relmrepo --force || exit 1
fi